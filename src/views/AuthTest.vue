    ...
    <n-card title="认证系统测试" :bordered="false">
      <n-space vertical size="large">
        ...
        <n-card title="API请求测试" size="small">
          <n-space vertical>
            <n-space>
              ...
              <n-button @click="testPublicApi" :loading="publicApiLoading"> 测试公开API </n-button>
            </n-space>
        ...
        <n-card title="认证操作" size="small">
          <n-space>
            <n-button @click="clearAuth" type="warning"> 清除认证信息 </n-button>
            <n-button @click="redirectToLogin" type="error"> 强制跳转登录 </n-button>
            <n-button @click="simulateTokenExpiry" type="error"> 模拟Token过期 </n-button>
          </n-space>
        </n-card>
        ...
        <n-card title="操作日志" size="small">
          <n-scrollbar style="max-height: 300px">
            <div v-for="(log, index) in logs" :key="index" class="log-item">
              <n-text :type="log.type"> [{{ log.time }}] {{ log.message }} </n-text>
            </div>
          </n-scrollbar>
